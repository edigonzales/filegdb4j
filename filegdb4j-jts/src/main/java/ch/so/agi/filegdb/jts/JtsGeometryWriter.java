 ...
 *
 * <p>Polygon rings are written with the Esri orientation convention: exterior
 * rings clockwise, interior rings counter clockwise. Z and M ordinates are
...
public final class JtsGeometryWriter {
  ...
  public FileGdbPolygon polygon(Polygon polygon) {
    ...
    parts.add(part(polygon.getExteriorRing().getCoordinates(), true));
    for (int i = 0; i < polygon.getNumInteriorRing(); i++) {
      parts.add(part(polygon.getInteriorRingN(i).getCoordinates(), false));
    }
    return new FileGdbPolygon(parts);
  }

  public FileGdbPolyline lineString(LineString lineString) {
    return new FileGdbPolyline(List.of(part(lineString.getCoordinates(), false)));
  }
  ...
  public ch.so.agi.filegdb.geometry.FileGdbGeometry write(Geometry geometry) {
    ...
    return switch (geometry.getGeometryType()) {
      ...
      case "MultiLineString", "MultiCurve" -> {
        List<FileGdbPart> parts = new ArrayList<>();
        for (int i = 0; i < geometry.getNumGeometries(); i++) {
          parts.add(part(((LineString) geometry.getGeometryN(i)).getCoordinates(), false));
        }
  ...
  private static FileGdbPoint point(Coordinate coordinate) {
    return new FileGdbPoint(
        coordinate.getX(), coordinate.getY(), z(coordinate), m(coordinate));
  }
  ...
  private static Double m(Coordinate coordinate) {
    return coordinate instanceof CoordinateXYZM xyzm ? xyzm.getM() : null;
  }

  private FileGdbPart part(Coordinate[] coordinates, boolean exterior) {
    List<Coordinate> values = new ArrayList<>(List.of(coordinates));
    if (!values.isEmpty()
        && !values.get(0).equals2D(values.get(values.size() - 1))) {
